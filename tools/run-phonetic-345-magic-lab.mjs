...
const permutations=+(process.env.M345_PERMS||120);
const maxWords=+(process.env.M345_WORDS||3000);
const concurrency=+(process.env.M345_CONCURRENCY||5);
const scope=process.env.M345_SCOPE||'full';

// Full mode deliberately attempts every canonical language used by the rest of the benchmark.
// Explicit mode remains available only as a diagnostic sensitivity subset.
let rows=[...metas];
...
console.log(`Running exact page engine ${Core.VERSION_V2}: ${rows.length}/${metas.length} canonical languages, ${permutations} controls/layer, ${maxWords} words/language`);
const result=await Core.runExperimentFast({
  ...
  onProgress:x=>{if(x.done%10===0||x.done===x.total)console.log(`${x.done}/${x.total} · ${x.results} completed · ${x.failures} failed`)}
});
result.generatedUtc=new Date().toISOString();
result.source='exact same phonetic-345-magic-core-v2 engine as the page tool';
result.canonicalLanguagesAvailable=metas.length;
