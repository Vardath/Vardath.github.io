...
OUT=ROOT/'data/literal-reverse-meaning-vertical-v2.json'
...
def main():
        ...
        for b in rtl:
            ...
            for c in shared:
                ...
                rows.append({'concept_id':c,'concept':labels.get(c,c),'reverse_distance':rev,'direct_distance':direct,
                             ...
                             'reverse_gain_vs_direct':direct-rev,'_wrong_reverse_distances':nd,
    ...
    # Pair-search-corrected permutation. Each concept already has 20 deterministic
    # wrong-meaning distances. A null universe draws one of those controls per concept,
    # then keeps the best apparent LR/RTL pair. This is the same wrong-meaning null used
    # by the primary score without recomputing millions of identical edit distances.
    pair_null=[]
    for p in range(PERMS):
        ...
        for q in horizontal:
            draws=[]
            for r in q['_rows']:
                nd=r['_wrong_reverse_distances']
                draws.append(nd[D.h64('litrev-perm-fast',p,q['LR'],q['RL'],r['concept_id'])%len(nd)])
            vals.append(mean(draws))
    ...
    for q in horizontal:
        raw=q.pop('_rows')
        for r in raw:r.pop('_wrong_reverse_distances',None)
        q['top_reverse_same_meaning_examples']=sorted(raw,key=lambda r:(r['semantic_advantage']+r['reverse_gain_vs_direct']),reverse=True)[:40]
