...
MIN_HALF_FAMILIES=4
FROZEN_SURVIVORS=[
 '473_december','158_cuckoo','462_january','479_saturday','1003_measure','203_mother','456_month','440_arrow','1008_draw',
 '334_picture','202_father','606_here','496_round','361_milk','307_house','836_build','641_thou','358_salt','362_tea',
 '464_march','984_chat','221_mind','570_familiar','841_bow','894_break','744_relax','548_clever'
]
...
def rawdist(a,b):return V1.edit(a,b,None,None,True)
def agg():return {'cases':0,'auc_real':0.0,'auc_label':0.0,'auc_random':0.0,'auc_raw':0.0,'top1_real':0.0,'top1_label':0.0,'top1_random':0.0,'top1_raw':0.0}
def addagg(a,m):
    a['cases']+=1
    for k,v in m.items():a[k]+=v
def avgagg(a):
    n=a['cases'];return {k:(v/n if k!='cases' and n else v) for k,v in a.items()}

def prepare():
    V1.prepare();x=V1.loadgz(V1.PREP)
    survivors=[c for c in FROZEN_SURVIVORS if c in x['concepts']]
    if len(survivors)!=27:raise RuntimeError(f'expected 27 frozen v1 FDR survivors, found {len(survivors)}')
    order=sorted(survivors,key=lambda c:(D.h64('residual-v2-shard',c),c));concept_shard={c:i%SHARDS for i,c in enumerate(order)}
    ...
    out={'version':2,'source':x['source'],'phones':x['phones'],'languages':x['languages'],'forms':x['forms'],'concept_labels':x['concept_labels'],'plan':x['plan'],'survivors':survivors,'concept_shard':concept_shard,'shard_sizes':shard_sizes,'thresholds':SENS_THRESH,'primary_threshold':PRIMARY_THRESH,'protocol':{
      ...
      'frozen_survivors':'The exact 27 semantic FDR survivors from the completed v1 concept test are hard-coded before v2 scoring.',
      ...
      'primary_threshold':PRIMARY_THRESH,'sensitivity_thresholds':SENS_THRESH,
    ...
    dumpgz(PREP,out);print(json.dumps({'survivors':len(survivors),'shard_sizes':shard_sizes,'thresholds':SENS_THRESH},indent=2))
...
def make_case(x,lid,concept,thr,cache):
    ...
    pool=V1.pool_for(x,lid,2000,cache);tcog=target.get('cog') or ''
    true_refs=filtered_refs(target['seq'],pool.get(concept,[]),thr,tcog)
    if len(true_refs)<2:return None
    ordered=sorted((c for c in pool if c!=concept),key=lambda c:(D.h64('residual-v2-neg',thr,lid,concept,c),c))[:NEG_SEARCH]
    negrefs=[]
    for c in ordered:
        rr=filtered_refs(target['seq'],pool.get(c,[]),thr,None)
        if len(rr)>=2:
            negrefs.append(rr)
            if len(negrefs)==NEG:break
    if len(negrefs)<NEG:return None
...
def concept_row(label,byfam):
    ...
    def havg(rows,k1,k2):return statistics.mean(r[k1]-r[k2] for r in rows) if rows else None
    hs={str(h):havg(rows,'auc_real','auc_label') for h,rows in halves.items()};hg={str(h):havg(rows,'auc_real','auc_random') for h,rows in halves.items()}
    eligible=overall['cases']>=MIN_CASES and len(famrows)>=MIN_FAMILIES and all(len(halves[h])>=MIN_HALF_FAMILIES for h in (0,1))
    return {'label':label,'cases':overall['cases'],'target_families':len(famrows),'half_family_counts':{str(h):len(halves[h]) for h in (0,1)},'metrics':overall,
      'semantic_auc_advantage':overall.get('auc_real',0)-overall.get('auc_label',0),'geometry_auc_advantage':overall.get('auc_real',0)-overall.get('auc_random',0),
      'semantic_half_advantage':hs,'geometry_half_advantage':hg,'semantic_family_wins':sum(d>0 for d in sem),'semantic_family_nonzero':len(sem),'semantic_p':signp(sum(d>0 for d in sem),len(sem)),
      'geometry_family_wins':sum(d>0 for d in geo),'geometry_family_nonzero':len(geo),'geometry_p':signp(sum(d>0 for d in geo),len(geo)),'eligible':eligible}

def shard(i):
    x=loadgz(PREP);concepts=[c for c in x['survivors'] if x['concept_shard'][c]==i]
    if not concepts:raise RuntimeError(f'empty shard {i}')
    real=V1.qlookup(x['phones'],GRID,False);rand=V1.qlookup(x['phones'],GRID,True);cache={};out={'shard':i,'concept_count':len(concepts),'thresholds':{}}
    ...
    WORK.mkdir(parents=True,exist_ok=True);(WORK/f'result-{i:02}.json').write_text(json.dumps(out,separators=(',',':')));print({'shard':i,'concepts':len(concepts)})

def merge():
    x=loadgz(PREP);shards=[json.loads((WORK/f'result-{i:02}.json').read_text()) for i in range(SHARDS)];results={}
    for thr in SENS_THRESH:
        rows={}
        for s in shards:rows.update(s['thresholds'][str(thr)])
        eligible=[c for c in sorted(rows) if rows[c]['eligible']];sq=bh_q([rows[c]['semantic_p'] for c in eligible]);gq=bh_q([rows[c]['geometry_p'] for c in eligible])
        ...
        results[str(thr)]={'eligible':len(eligible),'semantic_supported':supported,'geometry_supported':geometry,'both_supported':sorted(set(supported)&set(geometry)),'pooled':avgagg(pooled),'concepts':rows}
    primary=results[str(PRIMARY_THRESH)];summary={'frozen_survivors':len(x['survivors']),'primary_threshold':PRIMARY_THRESH,'primary_eligible':primary['eligible'],'primary_semantic_supported':len(primary['semantic_supported']),'primary_geometry_supported':len(primary['geometry_supported']),'primary_both_supported':len(primary['both_supported']),'semantic_survivor_labels':[primary['concepts'][c]['label'] for c in primary['semantic_supported']],'geometry_survivor_labels':[primary['concepts'][c]['label'] for c in primary['geometry_supported']],'sensitivity_counts':{t:{'eligible':r['eligible'],'semantic':len(r['semantic_supported']),'geometry':len(r['geometry_supported']),'both':len(r['both_supported'])} for t,r in results.items()}}
    out={'version':2,'status':'complete','test':'27-concept residual semantics after near-identical cross-family lexical-form exclusion','source':x['source'],'shards':SHARDS,'shard_sizes':x['shard_sizes'],'protocol':x['protocol'],'summary':summary,'results':results}
    OUT.write_text(json.dumps(out,indent=2),encoding='utf8');print(json.dumps(summary,indent=2))

def main():
    ap=argparse.ArgumentParser();sp=ap.add_subparsers(dest='cmd',required=True);sp.add_parser('prepare');q=sp.add_parser('shard');q.add_argument('--id',type=int,required=True);sp.add_parser('merge');a=ap.parse_args()
