...
def prepare():
    ...
    for lid in all_lids:
        ...
        plan[lid]={'shard':None,'same_ref':same_ref,'blocked_refs':blocked}
    # Deterministic hash-sorted round-robin assignment: 106 languages -> 20 populated shards.
    even_order=sorted(all_lids,key=lambda lid:(D.h64('lexsem-even-order',lid),lid))
    for j,lid in enumerate(even_order):plan[lid]['shard']=j%SHARDS
    x={'version':1,'source':{'phoible_commit':D.PH,'northeuralex_commit':NE_SHA},'phoible_coverage':cov,
       ...
       'protocol':{
        ...
        'shards':'20 deterministic populated language shards assigned round-robin after hash sorting; no language appears in two shards.',
    ...
    dumpgz(PREP,x);print(json.dumps(x['counts'],indent=2));print({'shard_sizes':[sum(1 for p in plan.values() if p['shard']==i) for i in range(SHARDS)]})
...
def shard(i):
    ...
    if not lids:raise RuntimeError(f'empty shard {i}')
...
def support(ss,mode,n,a,b,metric='top1'):
    ...
    wins=sum(x>0 for x in dif);p=signp(wins,len(dif))
    return {'mean_difference':statistics.mean(dif) if dif else None,'wins':wins,'valid_shards':len(dif),'sign_p':p,'supported':bool(dif and statistics.mean(dif)>0 and len(dif)==SHARDS and wins>=15 and p<.05)}

def merge():
    x=loadgz(PREP);ss=[json.loads((WORK/f'result-{i:02}.json').read_text()) for i in range(SHARDS)]
    if any(not s['languages'] for s in ss):raise RuntimeError('merge requires 20 populated shards')
    modes=['within_family','family_blocked'];summary={}
    for m in modes:
        traj=[]
        for n in RES:
        ...
        rw=sum(v>0 for v in rhos);rp=signp(rw,len(rhos));progress=bool(len(rhos)==SHARDS and statistics.mean(rhos)>0 and rw>=15 and rp<.05)
    ...
    OUT.write_text(json.dumps(out,indent=2));print(json.dumps({m:{'semantic_onset':v['semantic_onset'],'geometry_onset':v['geometry_onset'],'peak_resolution':v['peak_resolution'],'rho':v['rho_resolution_top1_mean'],'progression':v['meaning_refinement_progression_supported'],'raw_top1':v['raw_identity']['top1']} for m,v in summary.items()},indent=2))
