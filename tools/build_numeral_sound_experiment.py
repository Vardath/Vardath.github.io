...
KEEP=set(str(i) for i in range(0,21))
...
def latinish(s):
    ...
    return sum("a"<=c<="z" for c in x)/len(x) >= .8
...
params={r["ID"]:r for r in rows("parameters.csv")}
forms=[]
for r in rows("forms.csv"):
    ...
    forms.append({
        ...
        "latin_proxy":latinsh if False else latin-ish
    })

# fix generated flag without relying on variable-name tricks
for x in forms:
    x["latin_proxy"]=latinish(x["form"])

by_num=defaultdict(list)
for x in forms: by_num[x["number"]].append(x)

# Same-number family and cross-family form similarity, Latin-script proxy only.
...
for n,arr in sorted(by_num.items()):
    ...
    for i,a in enumerate(usable):
        for b in usable[i+1:]:
            s=sim(a["form"],b["form"])
            if a["family"]==b["family"] and a["family"]!="Unclassified": sf.append(s); same_family.append(s)
            elif a["family"]!=b["family"]: cf.append(s); cross_family.append(s)
...
# Null: cross-family forms with DIFFERENT numeral meanings. Deterministic sample.
rng=random.Random(137)
latin=[x for x in forms if x["latin_proxy"] and len(norm(x["form"]))>=2]
null=[]
for _ in range(min(50000,max(10000,len(cross_family)*2))):
    a,b=rng.sample(latin,2)
    if a["number"]==b["number"] or a["family"]==b["family"]: continue
    null.append(sim(a["form"],b["form"]))

# strongest same-number, different-family form lookalikes. Exploratory only.
out=[]
for n,arr in sorted(by_num.items()):
    usable=[x for x in arr if x["latin_proxy"] and len(norm(x["form"]))>=2]
    cand=[]
...
# Include the established eight calibration explicitly; these are historical controls,
# not inferred from UniNum spelling similarity.
eight_controls=[
...
same_mean_cross=mean(cross_family); diff_mean_cross=mean(null)
result={
 ...
 "method":{"warning":"UniNum Form/Segments are lexical/transcription data and are not assumed to be universal IPA. Form similarity is a Latin-script proxy only.","normalization":"Unicode NFKD, remove combining marks/nonletters; Levenshtein similarity","null":"different-number, different-family Latin-script form pairs, deterministic seed 137"},
 "summary":{"forms_in_scope":len(forms),"languages_in_scope":len(set(x['language_id'] for x in forms)),"families_in_scope":len(set(x['family'] for x in forms)),"same_family_same_number_mean":mean(same_family),"cross_family_same_number_mean":same_mean_cross,"cross_family_different_number_null_mean":diff_mean_cross,"same_number_minus_null":None if same_mean_cross is None or diff_mean_cross is None else same_mean_cross-diff_mean_cross,"same_family_minus_cross_family":None if mean(same_family) is None or same_mean_cross is None else mean(same_family)-same_mean_cross,"same_family_pairs":len(same_family),"cross_family_same_number_pairs":len(cross_family),"null_pairs":len(null)},
 "per_number":per_num,
 "forms":forms,
